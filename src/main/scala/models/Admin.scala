...
object Admin {
  ...
  def all()(implicit client: Client): Future[Seq[Admin]] = client.select("SELECT * FROM admins")(convertToEntity)

  def find(id: Long)(implicit client: Client): Future[Option[Admin]] =
    client.prepare("SELECT * FROM admins WHERE id = ?")(id) map { result =>
  ...
  def create(email: String, digest: String)(implicit client: Client): Future[Long] =
    client.prepare("INSERT INTO admins (email, digest) VALUES(?, ?)")(email, digest) map { result =>
